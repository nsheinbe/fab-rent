...
export function PayoutButtons({ payoutId, status, compact }: { payoutId: string; status: string; compact?: boolean }) {
  ...
  const run = (a: "retry" | "remind" | "release" | "mark_paid") => start(async () => { const r = await payoutAction(payoutId, a); toast({ title: r.ok ? (a === "retry" ? "Payout re-queued" : a === "remind" ? "Reminder sent" : a === "mark_paid" ? "Marked paid · provider notified" : "Payout released") : r.error, tone: r.ok ? "ok" : "error" }); router.refresh(); });
  ...
  if (status === "scheduled" && !compact) return <div className="flex gap-1.5"><Button size="sm" variant="secondary" onClick={() => run("mark_paid")} loading={pending} data-testid="mark-payout-paid">Mark paid</Button></div>;
  return null;
}

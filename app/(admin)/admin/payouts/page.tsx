    ...
    <div className="flex flex-col gap-5 px-4 py-5 md:px-6 lg:px-7">
      ...
      <section className="card overflow-hidden">
        <div className="overflow-x-auto"><div className="min-w-[860px]">
          ...
          {rows.map((r) => (
            <div key={r.id} className="grid grid-cols-[minmax(0,1.4fr)_100px_90px_110px_minmax(0,1.6fr)_110px_150px] items-center gap-3 border-b border-border px-4 py-2.5 text-[13px] last:border-b-0">
              <div className="min-w-0"><div className="truncate-1 font-semibold">{r.provider}</div><div className="truncate-1 text-[11px] text-text-3">{r.account_masked ?? "no account"}{!r.tax_id_verified ? " · tax ID missing" : ""}{!r.payout_account_verified ? " · bank unverified" : ""}</div></div>
              ...
              <span className="truncate-1 text-text-2">{r.exception ? `${r.exception}${r.exception_detail ? ` · ${r.exception_detail}` : ""}` : r.paid_at ? `paid ${formatDate(r.paid_at, tz)}` : "—"}</span>
